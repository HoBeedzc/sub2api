...
| **线下收款** | 管理员手动确认 | 用户先下单，支付由管理员在后台确认 |
...
5. 用户即可在前端页面进行充值
6. 如果启用了 **线下收款**，管理员可以在订单列表中手动确认收款并触发到账
...
线下收款不需要任何网关凭证。用户创建订单后，订单会先保持 `PENDING`，管理员收到线下转账后在后台订单页点击 **确认线下收款** 即可：

- 支付方式会在用户订单记录中显示为 `线下收款`
- 确认后系统会自动把订单标记为已支付并继续执行余额到账或订阅开通
- 不走第三方网关退款流程，退款需要人工线下处理
...
> 将 `your-domain.com` 替换为你的实际域名。EasyPay / 支付宝 / 微信的回调地址在添加服务商时自动填入，无需手动配置。线下收款不需要 Webhook。
...
- 确保服务器防火墙允许支付平台的回调请求
- 系统会自动进行签名验证，防止伪造回调
- 在线支付成功后自动完成余额充值；线下收款需要管理员先确认收款
...
> 线下收款订单同样会先创建为 `PENDING`。管理员在后台确认收款后，系统会自动把订单推进到 `PAID → RECHARGING → COMPLETED`。
...
| 支付方式 | EasyPay、支付宝、微信、Stripe | 相同，并新增线下收款 |

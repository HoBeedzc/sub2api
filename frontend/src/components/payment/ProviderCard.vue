  ...
  >
    <div :class="[
      ...
      <div class="flex items-center gap-4">
        ...
        <ToggleSwitch v-if="provider.provider_key !== 'offline'" :label="t('admin.settings.payment.refundEnabled')" :checked="provider.refund_enabled" @toggle="emit('toggleField', 'refund_enabled')" />
        <ToggleSwitch v-if="provider.provider_key !== 'offline' && provider.refund_enabled" :label="t('admin.settings.payment.allowUserRefund')" :checked="provider.allow_user_refund" @toggle="emit('toggleField', 'allow_user_refund')" />
...
const PROVIDER_KEY_LABELS: Record<string, string> = {
  ...
  offline: 'admin.settings.payment.providerOffline',
}

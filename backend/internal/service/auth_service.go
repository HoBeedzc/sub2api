...
func (s *AuthService) RegisterWithVerification(ctx context.Context, email, password, verifyCode, promoCode, invitationCode, affiliateCode string) (string, *User, error) {
	...
	invitation, err := s.resolveSignupInvitation(ctx, invitationCode, affiliateCode)
	if err != nil {
		return "", nil, err
	}
	...
	if err := s.createUserWithSignupInvitation(ctx, user, invitation); err != nil {
		// 优先检查邮箱冲突和注册策略错误（竞态条件下可能发生）
		switch {
		case errors.Is(err, ErrEmailExists):
			return "", nil, ErrEmailExists
		case errors.Is(err, ErrEmailDomainRegistrationLimit):
			return "", nil, ErrEmailDomainRegistrationLimit
		case errors.Is(err, ErrInvitationCodeInvalid), errors.Is(err, ErrInvitationCodeRequired):
			return "", nil, err
		default:
	...
	s.bindOptionalAffiliateAfterSignup(ctx, user.ID, affiliateCode, invitation)
	// 应用优惠码（如果提供且功能已启用）
...
func (s *AuthService) loginOrRegisterOAuthWithTokenPair(ctx context.Context, email, username, invitationCode, affiliateCode, promoCode, signupSource string) (*TokenPair, *User, error) {
	...
	if err != nil {
		if errors.Is(err, ErrUserNotFound) {
			...
			if err := s.createUserWithSignupInvitation(ctx, newUser, invitation); err != nil {
				if errors.Is(err, ErrEmailExists) {
					user, err = s.userRepo.GetByEmail(ctx, email)
					if err != nil {
						logger.LegacyPrintf("service.auth", "[Auth] Database error getting user after conflict: %v", err)
